...
BOOL getPlay(GOBAN current, int* i, int* j, MOKU stone, GOBAN previous, GOBAN *next, NN nn) { // and 2 workboards
...
void backpropagation(GOBAN position, NN nn, BOOL hasWon, float alpha, GOBAN deltaBoard1, GOBAN deltaBoard2) {
	MOKU value, aux, aux2, delta;
	int layer, i, j, fi, fj;
	GOBAN deltaBoardA, deltaBoardB;
	getValueGoban(position, nn, TRUE);
	for (layer = nn->size - 1; layer >= 0; layer--) {
		...
		for (i = 0; i < GOBAN_SIZE; i++) {
			for (j = 0; j < GOBAN_SIZE; j++) {
				if (layer < nn->size - 1) {
					delta = 0;
					getMoku(nn->layers[layer + 1]->goban, i, j, &aux2);

					for (fi = -1; fi < 2; fi++) {
						for (fj = -1; fj < 2; fj++) {
							getMoku(deltaBoardA, i + fi, j + fj, &aux);
							delta += aux * nn->layers[layer + 1]->weights[i][j][fi + 1][fj + 1];
							nn->layers[layer]->weights[i][j][fi + 1][fj + 1] -= alpha * aux * aux2;
						}
					}
				} else {
					delta = hasWon ? -1 : 1;
				}
				nn->layers[layer]->bases[i][j] -= alpha * aux;
				getMoku(nn->layers[layer]->goban, i, j, &aux);
				setMoku(deltaBoardB, i, j, delta * aux * (1 - aux));
			}
		}
		// printf("Layer %d\n", layer);
		// printValues(deltaBoardB);
		// printf("\n\n");
		// if (getMoku(deltaBoardB, (int[2]) {2, 2}) != getMoku(deltaBoardB, (int[2]) {2, 2})) {
		// 	exit(0);
		// }
	}
}
